...
def run_game_with_ML(model, display, clock):
    ...
    avg_score = 0 # by default zero
    test_games = 100  # total games 
    steps_per_game = 200

    for _ in range(test_games): # game testing function
        snake_start, snake_position, apple_position, score = starting_positions()  #initial location

        count_same_direction = 0
        prev_direction = 0

        for _ in range(steps_per_game): # move snake in possible direction
...
tkinter.messagebox.showinfo(title = "Average Score " , message = "Your average score is " + str(avg_score))